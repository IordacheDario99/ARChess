...
public class SaveInstantTarget : MonoBehaviour {
    ...
    /// <summary>
    /// Saves the instant target to disk. This only saves the point cloud representation of the scene,
    ...
    /// <summary>
    /// Saves all augmentations to disk. Each augmentation is represented by an AugmentationDescription and
    ...
    /// </summary>
    /// <param name="augmentation">The GameObject for which the ID is required.</param>
    /// <returns>Index of the augmentation in InstantTrackingController.Models. Returns -1 if not found.</returns>
    private int GetAugmentationId(GameObject augmentation) {
        for (int i = 0; i < Controller.Models.Count; ++i) {
            // Because instantiated prefabs have the "(Clone)" suffix applied, we only check if the names
            // start the same way.
